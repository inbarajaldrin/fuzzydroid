...
uv pip install --python .venv/bin/python -e shared/fuzzydroid
...
```bash
.venv/bin/python -m pytest shared/fuzzydroid/tests -v
```
...
- Tests required for every new module in `shared/fuzzydroid/fuzzydroid/common/` and `shared/fuzzydroid/fuzzydroid/<plugin>/`.
